 ...
 */
package com.hcb.hcbsdk.logutils.upload;
...
public abstract class BaseUpload implements ILogUpload {
    ...
    public String buildTitle(Context context) {
        return "【CrashLog】  " + context.getString(context.getApplicationInfo().labelRes) + " "+
                getUserTel()+" " + yyyy_MM_dd_HH_mm_ss_SS.format(Calendar.getInstance().getTime());
    }

    public String getUserTel(){
        String name = "";
        if(SDKManager.getInstance().getUser()!=null)
            name = SDKManager.getInstance().getUser().getNickname()+" "+SDKManager.getInstance().getUser().getMobile();
        return name;
