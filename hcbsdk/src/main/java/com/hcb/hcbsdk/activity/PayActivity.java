 ...
 * @updateDes ${TODO}
 */
public class PayActivity extends JKCBaseActivity {
    ...
    private void initData(OrderForm mOrder) {
        ...
        tv_need_pay.setText(""+totalpayNum);
        ...
        if(payType.equals("gold")){//金币
            ...
            if(hasPayNum>0) {//用户金币不够，需充值剩余金币
                tv_title.setText("金币购买");
                tv_chang1.setText("还需购买 ");
                tv_need_pay.setText(""+(((float) hasPayNum)*1000));
            }else{
                tv_title.setText("金币支付");
                tv_chang1.setText("确认支付 ");
            }
            tv_chang2.setText("金币");

        }else if(payType.equals("bean")){//菜豆
            tv_chang3.setVisibility(View.VISIBLE);
            if(hasPayNum>0) {//用户金币不够，需充值剩余金币
                tv_title.setText("菜豆购买");
                tv_chang1.setText("还需购买 ");
                tv_need_pay.setText(""+(((float) hasPayNum)*100));
                tv_chang3.setVisibility(View.VISIBLE);
                tv_chang3.setText(" 送积分"+(((float) hasPayNum)*100));
            }else{
                tv_chang3.setVisibility(View.GONE);
                tv_title.setText("菜豆支付");
                tv_chang1.setText("确认支付 ");
            }
            tv_chang2.setText("菜豆");

        }else if(payType.equals("point")){//积分
            tv_chang3.setVisibility(View.VISIBLE);
            if(hasPayNum>0) {//用户金币不够，需充值剩余金币
                tv_chang3.setText(" 获取积分"+(((float) hasPayNum)*100));
                tv_need_pay.setText(""+(((float) hasPayNum)*100));
                tv_title.setText("积分获取");
                tv_chang1.setText("还需购买 ");
                tv_chang2.setText("菜豆");
            }else{
                tv_chang3.setVisibility(View.GONE);
                tv_title.setText("积分支付");
                tv_chang1.setText("确认使用 ");
            ...
            public void onSuccess(Object responseObj) {
                ...
                try {
                    ...
                    if(status == 10000){
                        ...
                        if(mOrder.getData().getCash()==0){//用户金币足够
                        ...
                        Utils.showToastCenter(PayActivity.this, "网络差，无法完成该操作~");
                        iv_net_error.setVisibility(View.VISIBLE);
                    }
            ...
            public void onSuccess(Object responseObj) {
                ...
                try {
                    ...
                    if(status == 10000){
                        ...
                        BroadcastUtil.sendBroadcastToUI(PayActivity.this, IConstants.PAY_SUCCESS,((JSONObject)responseObj).toString());

                        Utils.showToastCenter(PayActivity.this, "支付成功");
                        finish();
                    }
                    else
                        Utils.showToastCenter(PayActivity.this, "网络差，无法完成该操作~");
